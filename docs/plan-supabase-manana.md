...
- [x] Evaluar el free tier en este proyecto; decidir si hace falta Starter ($19/mes) o Standard ($69/mes) según volumen — **pendiente de decisión del usuario: quedan ~150 créditos.**
- [x] Fix TC018 (bug real): soft delete de clientes ahora expuesto en la UI como **"Desactivar cliente"** (decisión de negocio: los clientes NUNCA se eliminan, solo se desactivan preservando historial). Botón en la ficha (client-sheet.tsx) + `useDeleteClient` → `DELETE /api/v1/clients/:id` (deleted_at=now). **Verificado con smoke real: count 3→4 al crear, DELETE 204, count 4→3, detalle 404 — el registro queda en DB.** TC028 requiere cuenta fresca para el empty state.
