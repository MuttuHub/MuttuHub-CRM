...
- [x] GitHub Action de TestSprite como gate en PRs — **`.github/workflows/ci.yml` (npm test + lint, gate de PR siempre activo) + `.github/workflows/testsprite.yml` (E2E TestSprite/run-action@v1, blocking; preview deploy de Vercel en PRs si existe el secret `VERCEL_TOKEN`, si no se skipea con aviso; en push a main corre contra producción). Secrets: `TESTSPRITE_API_KEY` cargado. Pendiente: crear `VERCEL_TOKEN` en el dashboard de Vercel para activar el gate E2E en PRs (la integración Git sigue rota, los deploys son por CLI).**
