// Helpers compartidos del módulo Dashboard (Hito 6, PRD §7): resolución de
// alcance, parseo de los filtros comunes (§7.2) y constructores de `where`
// base para las cuatro caras (pipeline, tasks, clients-activity, my-summary).
//
// Alcance (documentado en cada ruta): COLABORADOR → "own" (tareas:
// responsable_id = self; clientes: responsable_id = self; oportunidades:
// cliente.responsable_id = self); el resto de roles → "all" (plataforma
// completa). Cuando un filtro pasa `responsable_id` explícito, gana en scope
// "all"; en scope "own" se ignora en silencio (documentado). Toda respuesta
// incluye `scope: "own" | "all"`.
//
...
/** Alcance de lectura del dashboard: COLABORADOR → "own"; resto → "all". */
export function resolveScope(usuario: Pick<Usuario, "rol">): DashboardScope {
  return isFullAccess(usuario.rol) ? "all" : "own";
}
