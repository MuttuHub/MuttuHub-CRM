...
function routeContext(id = "doc-1", versionId = "v-1") {
  return { params: Promise.resolve({ id, versionId }) };
}

function request(id = "doc-1", versionId = "v-1"): Request {
  return new Request(`http://localhost/api/v1/documents/${id}/versions/${versionId}/download`);
}
...
describe("GET /api/v1/documents/:id/versions/:versionId/download", () => {
  ...
  it("returns 404 when the version does not belong to the document", async () => {
